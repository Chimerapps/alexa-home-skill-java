 ...
 * Date 10/03/2017.
 */
public class SetPercentageRequest extends ApplianceRequest {

	private final Value percentageState;

	@JsonCreator
	public SetPercentageRequest(@JsonProperty("accessToken") final String accessToken,
	                            @JsonProperty("appliance") final Appliance appliance,
	                            @JsonProperty("percentageState") final Value percentageState) {
		super(accessToken, appliance);
		this.percentageState = percentageState;
	}

	public Value getPercentageState() {
